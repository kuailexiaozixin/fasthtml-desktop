# -*- coding: utf-8 -*-
"""desktop.py — FastMeet 桌面入口（pywebview + uvicorn）
...
要点：
  ...
  - 通过 FASTMEET_DB / FASTSME_AUTH_DB 把两个 SQLite 库重定向到可写目录
    （必须早于 `import web_app`：db.py 与 account_auth.py 在模块级读这两个变量）
  - 首次启动由上游 `_ensure_db()` 自动播种演示会议（幂等：库存在即跳过）
...
登录（两套并存）：
  Sign-in 弹窗 / 原生登录页 : admin@fastmeet.example / FastMeet2026$
"""
...
def start() -> None:
    ...
    print("[INFO] AI 议程/纪要需自备 LLM Key（OPENAI_API_KEY / ANTHROPIC_API_KEY / XAI_API_KEY）")
