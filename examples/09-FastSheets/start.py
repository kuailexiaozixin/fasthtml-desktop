# -*- coding: utf-8 -*-
"""start.py — FastSheets 一键启动脚本（开发/测试/调试/验证统一入口）
...
def main():
    ...
    if "--reseed" in args:
        for f in (HERE / "fastsheets.sqlite", HERE / "fastsme-accounts.sqlite"):
            if f.exists():
                f.unlink()
                print(f"[OK] 已删除 {f.name}，下次启动将重新播种")
            else:
                print(f"[INFO] {f.name} 不存在，无需重置")
        args.remove("--reseed")
        if not args:
            return
