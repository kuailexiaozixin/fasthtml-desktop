# -*- coding: utf-8 -*-
"""desktop.py — FastMSR 桌面入口（pywebview + uvicorn）
...
要点：
  ...
  - 通过 FASTMSR_DB / FASTSME_AUTH_DB 把两个 SQLite 库重定向到可写目录
...
登录（两套并存）：
  Sign-in 弹窗 / 原生登录页 : admin@fastmsr.example / FastMSR2026$
"""
...
def start() -> None:
    ...
    print("[INFO] Freddie Mac CRX 为**模拟**实现，不连任何外部系统")
