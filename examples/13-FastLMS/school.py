...
(assessments + grading scale), attendance, and fees. Lives in the same database
as db.py and follows its SQLAlchemy patterns (SQLite in the desktop bundle,
PostgreSQL upstream -- see db.py for the porting notes).
...
CREATE INDEX IF NOT EXISTS idx_sch_students_group ON students(group_id);
CREATE INDEX IF NOT EXISTS idx_sch_results_student ON assessment_results(student_id);
CREATE INDEX IF NOT EXISTS idx_sch_att_student ON attendance(student_id, att_date);
CREATE INDEX IF NOT EXISTS idx_sch_fees_student ON fees(student_id);
"""
...
def list_students(conn, group_id=None, q=None):
    ...
    if q:
        where.append("(s.first_name LIKE :q OR s.last_name LIKE :q OR s.code LIKE :q)"); params["q"] = f"%{q}%"
