...
account_auth.register_fasthtml_routes(rt, app_name="FastMail", session_key="user", success_path="/")

# Provision the documented demo admin so the login modal works without an email
# verification round-trip (the auth store otherwise starts empty and registration
# cannot verify accounts locally). Override with FASTMAIL_ADMIN_EMAIL/PASSWORD.
...
def suite_callback(session, ticket: str = ""):
    identity = suite_auth.redeem(ticket, "mail")
...
def _seed_needed() -> bool:
    """全新安装时是否需要播种。

    不能只看 db.db_exists()：`web/api.py` 在 import 时就通过
    `SQLiteBackend(..., initialize=db.init_schema)` 建好了空表，库文件因此已存在
    且非空字节，`db_exists()` 恒为真 —— 于是演示邮件永远播不进去，收件箱是空的。
    改为按「messages 表是否有行」判断。
    """
    if not db.db_exists():
        return True
    try:
        return not db.scalar("SELECT COUNT(*) FROM messages")
    except Exception:
        return True
...
def _ensure_db():
    if _seed_needed():
        logger.info("No messages found — seeding synthetic mailbox…")
        import seed
        seed.build()
    else:
