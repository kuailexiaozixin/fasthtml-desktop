# -*- coding: utf-8 -*-
"""desktop.py — FastESM 桌面入口（pywebview + uvicorn）
...
要点：
  ...
  - 通过 FASTESM_DB / FASTSME_AUTH_DB 把两个 SQLite 库重定向到可写目录
...
登录（两套并存）：
  Sign-in 弹窗 / 原生登录页 : admin@fastesm.example / FastESM2026$
"""
