...
from db import connect, reset_tables
...
# SQLite has no TRUNCATE; db.reset_tables() does the equivalent (DELETE plus a
# reset of the AUTOINCREMENT counter) with foreign keys suspended, so the order
# below is only about readability, not dependencies.
TRUNCATE_TABLES = [
    "agent_invocations",
    "dd_findings",
    "market_signals",
    ...
    # RAG side; `embeddings` is a vec0 virtual table and has no foreign keys,
    # so it must be wiped explicitly or its vectors would outlive their chunks.
    "rag_queries",
    "embeddings",
    "chunks",
    "documents",
