...
SQLite backend (see db.py for the PostgreSQL->SQLite porting notes),
XP + streaks + badges + leaderboard interactivity.

Usage:
    python main.py                     # http://localhost:5001
    python main.py --port 8000         # custom port
    python desktop.py                  # desktop window (pywebview)

Demo logins:
    Sign-in modal : admin@fastlms.example / FastLMS2026$
    /auth/login   : instructor@fastlms.dev / admin   (seeded by seed.py)
...
# Demo credentials for the FastSME sign-in modal (separate store from the native
# /auth/login form below). Provisioned as already-verified so the modal works
# offline -- local registration can never complete an email verification
# round-trip. Override with FASTLMS_ADMIN_EMAIL / FASTLMS_ADMIN_PASSWORD.
VALID_EMAIL = os.getenv("FASTLMS_ADMIN_EMAIL", "admin@fastlms.example")
VALID_PASSWORD = os.getenv("FASTLMS_ADMIN_PASSWORD", "FastLMS2026$")
account_auth.accounts.ensure_account(VALID_EMAIL, VALID_PASSWORD, "FastLMS Admin", verified=True)
    ...
    with db.begin() as conn:
        ...
        conn.execute(
            sa.text(f"""
                ...
                VALUES (:u, :q, :s, :p, :a, CURRENT_TIMESTAMP)
            """),
