# -*- coding: utf-8 -*-
"""main.py — FastSlides 桌面入口（pywebview + uvicorn）

将上游 web_app.py 的 FastHTML 应用包装为桌面应用：
  - 只读资源(_MEIPASS 顶层) 与可写数据(EXE 同级 data/) 分离
  - 通过 FASTSLIDES_DB 环境变量把 SQLite 重定向到可写目录
  - FASTSME_AUTH_DB 同步重定向到可写数据目录（frozen 模式 _MEIPASS 只读）
...
APP_TITLE = "FastSlides"
ENV_PREFIX = "FASTSLIDES"
DB_FILENAME = "fastslides.sqlite"
DEFAULT_PORT = 5013
...
# 认证库同样重定向到可写数据目录，避免 frozen 模式写只读 _MEIPASS 导致注册/登录失败
...
def start() -> None:
    ...
    print(f"[OK] FastSlides 服务启动: http://127.0.0.1:{port}")
    print("[INFO] 登录: admin@fastslides.example / FastSlides2026$")
