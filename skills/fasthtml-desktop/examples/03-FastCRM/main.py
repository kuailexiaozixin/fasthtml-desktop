# -*- coding: utf-8 -*-
"""main.py — FastCRM 桌面入口（pywebview + uvicorn）

将上游 web_app.py 的 FastHTML 应用包装为桌面应用：
  - 只读资源(_MEIPASS 顶层) 与可写数据(EXE 同级 data/) 分离
  - 通过 FASTCRM_DB 环境变量把 SQLite 重定向到可写目录
...
def start() -> None:
    ...
    print(f"[OK] FastCRM 服务启动: http://127.0.0.1:{port}")
    print("[INFO] 登录: admin@fastcrm.example / FastCRM2026$")
