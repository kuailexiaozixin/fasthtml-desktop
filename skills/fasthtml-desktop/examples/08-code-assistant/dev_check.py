# -*- coding: utf-8 -*-
"""dev_check.py — Code Assistant 质量门禁（子进程冒烟，不弹窗口）

启动上游 code_assistant 子进程，探测端口后 GET / 断言 200，再退出。
需要先用 启动.bat 或 `pip install -r requirements.txt` 装好依赖
（ast-grep-py / astra-assistants / python-fasthtml / python-dotenv / httpx）。
...
def main():
    env = dict(os.environ)
    env["CA_GENERATED_APPS_DIR"] = str(HERE / "generated_apps")
    proc = subprocess.Popen([sys.executable, "-m", "code_assistant.code_assistant"],
