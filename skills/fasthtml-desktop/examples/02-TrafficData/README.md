...
This example ships with a pywebview desktop shell so the dashboard runs in its
own window with **no server/Python setup for the end user** and **works fully
offline** (charts included):

```bat
启动.bat            :: double-click → auto-detect Python, install deps, open window
```

Or drive it from the launcher directly:

```bash
launcher.py server   :: run the HTTP server (headless-friendly)
launcher.py --check  :: dependency preflight only
SERVER_ONLY=1 python main.py   :: headless HTTP service (CI / smoke test)
```

**Offline charts.** The upstream dashboard loads `plotly.js` from a CDN
(`cdn.plot.ly`); disconnected machines would show blank charts. The desktop
shell copies the **plotly.min.js bundled inside the installed `plotly` package**
into `vendor/` on first launch and points the app at the local copy — zero
extra bytes committed, fully offline. If plotly is missing it safely falls back
to the CDN.

**No database, no login.** This is a read-only synthetic-data dashboard: open
the window and the six pages are immediately available. `vendor/` is generated
...
├── pyproject.toml
├── main.py          # desktop shell (pywebview + uvicorn); offline plotly bootstrap
├── launcher.py      # dependency-aware launcher (shared engine, byte-identical)
├── launcher.json    # per-example config (app name, run notes)
├── 启动.bat         # thin Windows dispatcher (GBK + CRLF)
├── dev_check.py     # in-process smoke test (starlette TestClient)
├── vendor/          # generated at runtime: local plotly.min.js (git-ignored)
└── README.md
```
