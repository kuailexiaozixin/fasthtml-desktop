...
# cannot verify accounts locally). Override with FASTMEET_ADMIN_EMAIL/PASSWORD.
...
def _seed_needed() -> bool:
    ...
    且非空字节，`db_exists()` 恒为真 —— 于是演示数据永远播不进去，应用起来是空的。
    改为按「meetings 表是否有行」判断。
    """
    if not db.db_exists():
        return True
    try:
        return not db.scalar("SELECT COUNT(*) FROM meetings")
    except Exception:
        return True
...
def _ensure_db():
    if _seed_needed():
        logger.info("No meetings found — seeding synthetic meetings…")
        import seed
        seed.build()
