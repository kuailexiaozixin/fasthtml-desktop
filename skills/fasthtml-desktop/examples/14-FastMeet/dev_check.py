# -*- coding: utf-8 -*-
"""dev_check.py — FastMeet 一键质量门禁（测试/调试/检查/验证）
...
# 门禁用独立库文件，避免污染用户的 data/fastmeet.sqlite 与账号库
os.environ.setdefault("FASTMEET_DB", str(HERE / "data" / "devcheck.sqlite"))
os.environ.setdefault("FASTSME_AUTH_DB", str(HERE / "data" / "devcheck-accounts.sqlite"))

ROUTES = ["/", "/meetings", "/meetings?scope=past", "/schedule", "/ai", "/guide"]
...
def main():
    ...
    new_email = f"gate-{uuid.uuid4().hex[:8]}@fastmeet.example"
    ...
    check("新注册账号可直接登录", r.status_code == 200 and r.json().get("redirect") == "/",
          f"status={r.status_code} body={r.text[:120]}")
    r = reg.get("/meetings", follow_redirects=False)
    check("新注册账号进入 /meetings", r.status_code == 200, f"status={r.status_code}")

    # 未登录拦截
    anon = TestClient(app)
    r = anon.get("/meetings", follow_redirects=False)
    check("未登录访问 /meetings 被拦截",
    ...
    # 会议详情 / 会议室（取库里第一场会议，验证演示数据已播种）
    import db
    rows = db.meetings("all")
    check("演示会议已播种", bool(rows), f"count={len(rows)}")
    if rows:
        mid = rows[0]["id"]
        for route in (f"/meeting/{mid}", f"/room/{mid}"):
            r = c.get(route, follow_redirects=False)
            check(f"弹窗登录态 GET {route}", r.status_code == 200, f"status={r.status_code}")
    ...
    r = nat.get("/", follow_redirects=False)
    check("原生登录态进入仪表盘", r.status_code == 200, f"status={r.status_code}")
