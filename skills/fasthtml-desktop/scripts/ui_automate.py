...
def _js_str(s: str) -> str:
    """把任意字符串安全转义为 JS 字符串字面量。

    用 json.dumps 生成合法 JS 字符串（含反斜杠 u 四连的 unicode 转义），
    彻底避免手工拼单引号字符串时因选择器/文本含单引号、反斜杠、中文而破坏 JS 语法。
    """
...
def _click(window, selector: str) -> bool:
    """派发真实 click 事件。"""
    js = ("(function(){var el=document.querySelector(" + _js_str(selector) +
...
def _type_text(window, selector: str, text: str) -> bool:
    ...
    js = ("(function(){var el=document.querySelector(" + _js_str(selector) +
           ");if(!el)return 'NOT_FOUND';el.value=" + _js_str(text) + ";" +
...
def _assert_text(window, selector: str, expected: str) -> bool:
    ...
    js = ("(function(){var el=document.querySelector(" + _js_str(selector) +
           ...
           "return {ok:t.indexOf(" + _js_str(expected) + ")>=0,text:String(t).slice(0,120)};})()")
    res = _eval(window, js)
    if isinstance(res, str):
        try:
            res = json.loads(res)
...
def _assert_attr(window, selector: str, attr: str, value: str) -> bool:
    ...
    js = ("(function(){var el=document.querySelector(" + _js_str(selector) +
           ");if(!el)return {ok:false,reason:'not found'};" +
           "var v=el.getAttribute(" + _js_str(attr) + ")||'';" +
           "return {ok:v===" + _js_str(value) + ",actual:v};})()")
    res = _eval(window, js)
    if isinstance(res, str):
        try:
            res = json.loads(res)
...
def _assert_count_gt(window, selector: str, min_count: int) -> bool:
    """断言选择器匹配数 > min_count。"""
    js = ("(function(){var els=document.querySelectorAll(" + _js_str(selector) +
